...
app.get('/info', (request, response) => {
    ...
    Person.find({}).then(persons => {
      response.send('<h1>Phonebook has info for ' +persons.length  +' people</h1><h1>' +fecha +'</h1>')
    })
...
app.post('/api/persons', (request, response, next) => {
  ...
  const na=body.name
  Person.find({name:na}).then(persons => {
    if (persons.length>0){
      return response.status(400).json({ 
          error: 'Then name ' +na +' is already exist. Name must be unique' 
          })
    }
  })
